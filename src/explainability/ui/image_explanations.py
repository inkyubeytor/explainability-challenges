...
def grad_cam(image, model):
    ...
    visualization = show_cam_on_image(np.moveaxis(image.numpy()[0] / 255, 0, -1), grayscale_cam, use_rgb=True)
...
def eigen_cam(in_image, og_image, model):
    ...
    image = np.ascontiguousarray(torch.moveaxis(image.squeeze().int(), 0, 2).numpy(), dtype=np.uint8)
    image_float_np = np.float32(image) / 255

    og_image = np.ascontiguousarray(torch.moveaxis(og_image.squeeze().int(), 0, 2).numpy(), dtype=np.uint8)
    ...
    grayscale_cam = grayscale_cam[0, :]
    cam_image = show_cam_on_image(og_image_float_np, grayscale_cam, use_rgb=True)
...
# image = torchvision.io.read_image("/content/test_img.JPEG").float().unsqueeze(dim=0)
...
# image = read_image("/content/test_img.JPEG").float().unsqueeze(dim=0).to("cpu")
